    ...
    require(['jquery', 'popdialog', 'tipdialog', 'toast', 'helper', 'common', 'formcontrol', 'prevNextpage', 'tableheadfix', 'system', 'selectcity', 'filelist', 'fileupload', 'selectCity2', 'metronic', 'customtable', 'bootstrap-datepicker.zh-CN', 'bootstrap-datetimepicker.zh-CN'],
        function ($, popdialog, tipdialog, toast, helper, common, formcontrol, prevNextpage, tableheadfix, system, selectcity, filelist, fileupload, selectCity2) {
            ...
            function getPingTaiDaiLiShang(id, callback) {
                helper.Ajax("003300300002", id, function (resultdata) {
            ...
            function fillFormData(resource) {
                $('#Form1').find('input[name],textarea[name]').each(function (i, item) {
                    $(item).val('');
                ...
                //多选下拉赋值
                $('input[name="JingYingQuYu"]').val(resource['JingYingQuYu']);
                ...
                if ($('#PingTaiDaiLiBiaoZhiId').val() != '') {
                    var url = helper.Route('00000080004', '1.0', system.ServerAgent) + '?id=' + $('#PingTaiDaiLiBiaoZhiId').val();
                ...
                if ($("#QiYeLeiXing").val() == 1) {
                    $("#QYLX").val("平台代理商");
                }
                else if($("#QiYeLeiXing").val() == 2) {
                    $("#QYLX").val("分支机构");
                }
                else if($("#QiYeLeiXing").val() == 3) {
                    $("#QYLX").val("道路运输企业");
                }else{
                        $("#QYLX").val("道路运输车队");
                      }
            ...
            function save() {
                ...
                //调用修改接口
                helper.Ajax("003300300005", jsonData1, function (data) {
                    if ($.type(data) == "string") {
            ...
            function imageLoad() {
                $('#imgUpLoad').fileupload({
                    ...
                    callback: function (data) {
                        $("#PingTaiDaiLiBiaoZhiId").val(data.FileId);
