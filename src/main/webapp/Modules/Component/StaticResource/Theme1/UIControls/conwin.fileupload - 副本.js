define(['jquery', 'system', 'toast', 'popdialog', 'warning', 'helper'], function ($, system, toast, popdialog, warning, CwHelper) {
    ...
    var defaults = {
        maxSize: '4',//文件大小限制
        allowedContentType: 'png|jpg|jpeg|tif|gif|pdf|doc|docs|xls|xlsx|ppt|pptx|txt|rar|zip|apk',//支持的文件格式
    ...
    $(window).on('message', function (e) {
        if (!isTimeout) {
            ...
            if (result.success) {
                ...
                warning.confirm("上传失败，" + result.msg, function (r) {
                    if (r) {
                        $('.file-caption-name').remove();
                        $('#fileUploadForm')[0].reset();
                    }
    ...
    fileupload.fileUpload = function (options, selectName, user) {
        ...
        $('#fileUploadModal_Upload').click(function (e) {
            ...
            timeout = setTimeout(function () {
                isTimeout = true;
                Metronic.unblockUI('#ajax-modal .modal-body');
                warning.confirm("上传超时，请重试！", function (r) {
                    if (r) {
                        $('.file-caption-name').remove();
                        $('#fileUploadForm')[0].reset();
                    }
    ...
    fileupload.rebindSingleFileButtonForViewAndEdit = function (operatorObj, fileId, isReadOnly, multi) {
        ...
        for (var i = 0; i < fileIdArray.length; i++) {
            ...
            if (!multi) {
                $("#" + operatorObj.ButtonName).parent().prepend(htmlStr);
                uploadFileBtnDom = $("#" + buttonName).detach();
            }
            else {
                //htmlStr = '<div style="margin-top:5px;">' + htmlStr + '</div>';
                $("#" + operatorObj.ButtonName).parent().prepend(htmlStr);
    ...
    var getFileHtml = function () {
        ...
        html += '<h4 class="modal-title">上传文件</h4>';
        html += '</div>';
        ...
        html += '<a href="javascript:void(0)" class="btn default" data-dismiss="modal"><i class="fa fa-close"></i>&nbsp;关闭</a>';
        html += '</div>';
        html += '</div>';
        html += '</div>';
