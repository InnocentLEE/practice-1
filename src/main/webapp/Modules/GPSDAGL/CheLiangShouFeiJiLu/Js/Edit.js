    ...
    require(['jquery', 'popdialog', 'tipdialog', 'toast', 'helper', 'common', 'formcontrol', 'prevNextpage', 'tableheadfix', 'system', 'selectcity', 'filelist', 'metronic', 'customtable', 'bootstrap-datepicker.zh-CN', 'bootstrap-datetimepicker.zh-CN'],
        function ($, popdialog, tipdialog, toast, helper, common, formcontrol, prevNextpage, tableheadfix, system, selectcity, filelist, Metronic) {
            ...
            var initPage = function () {
                ...
                $('#ShouFeiNeiRong').change(function (e) {
                    if ($(this).val() == 3) {//3=续网状态,可修改服务截至日期
                        $('#FuWuJieShuRiQi').attr('disabled', false);
                        $('#FuWuQiShiRiQi').attr('disabled', false);
                        ...
                        $('#FuWuJieShuRiQi').attr('disabled', true);
                        $('#FuWuQiShiRiQi').attr('disabled', true);
                    }
            ...
            function fillFormData(resource) {
                $('#Form1').find('input[name],select[name],textarea[name]').each(function (i, item) {
                    ...
                    if (tempValue != undefined) {
                        ...
                        //if ($(item).attr('name') == 'ChePaiYanSe') {
                        //    var colorStr = new Array('黄色', '黑色', '蓝色', '白色', '其他');
                        //    tempValue = colorStr[tempValue - 1];
                        //}
                        if ($(item).attr('name') == 'FuWuJieShuRiQi') {
                            tempValue = tempValue.substr(0, 10);
                            FuWuJieShuRiQi = tempValue;
                        } else if ($(item).attr('name') == 'FuWuQiShiRiQi') {
                            tempValue = tempValue.substr(0, 10);
                            FuWuJieShuRiQi = tempValue;
                        }
                        if ($(item).attr('name') == 'ShouFeiNeiRong') {
                            if (tempValue == 3) {
                                $('#FuWuJieShuRiQi').attr('disabled', false);
                                $('#FuWuQiShiRiQi').attr('disabled', false);
                                $('.datepicker').datepicker({
            ...
            function save() {
                ...
                var FuWuJieShuRiQiVal = $('#FuWuJieShuRiQi').val();
                if (jsonData1.ShouFeiNeiRong != 3) {//3=续完状态。
                    if (FuWuJieShuRiQiVal != FuWuJieShuRiQi) {
                        tipdialog.alertMsg("错误操作，收费内容为续完状态时，才可以修改服务截至日期！");
                        return false;
                    }
                }

                //调用修改接口
                helper.Ajax("003300300086", jsonData1, function (data) {
                    if (data.body) {
